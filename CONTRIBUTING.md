...
1. Read `docs/ARCHITECTURE.md`, `docs/MISSION_CATALOGUE.md` and the scoring contract in the README.
2. Run `npm ci`, then `npm run check`.
...
CI lives in `.github/workflows/ci.yml` and runs deterministic verification on pushes, pull requests and manual dispatch. Full Playwright browser qualification remains an explicit release gate rather than an automatic browser-download job. Do not store credentials, tokens, production URLs with access tokens, or personal gameplay data in commits.
